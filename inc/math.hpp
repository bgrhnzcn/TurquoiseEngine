...
// Norm
inline float norm(const Vec2 vec) { return std::sqrt(dot(vec, vec)); }
inline float norm(const Vec3 vec) { return std::sqrt(dot(vec, vec)); }
inline float norm(const Vec4 vec) { return std::sqrt(dot(vec, vec)); }
